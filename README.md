...
- **2026-06-15** — Design port deployed to production (Versus / Rounds / Log live on the real beton.css design).
...
- **2026-06-16** — Added **viewer analytics**: every dashboard load is logged server-side to `view_events` (throttled to ~30-min visits, competitors + observers), queryable via `v_viewer_activity`. DB objects live; recording starts once `app/dashboard/page.tsx` is deployed. No in-app UI.
- **2026-06-16** — Fixed the **Log day rolling over in UTC**: the dashboard computed "today" via `toISOString()` (UTC), so a new day card didn't appear until UTC midnight (2 am for CEST users). Now derives today from the local calendar date.
- **2026-06-16** — Log sheet number fields (**calories, protein, junk**) are now **directly typeable** — tap to type a value (numeric keypad on mobile) in addition to the +/- steppers and quick-add chips.
- **2026-06-16** — Added **Extended walk** as a gym activity (new `muscle_group` enum value), shown with a 🧑‍🦯 emoji in the gym grid and day-card chips. Emoji is display-only (stored value stays `Extended walk`).
